...
abstract class BaseaMediaItemForm extends BaseFormDoctrine
{
  public function setup()
  {
    $this->setWidgets(array(
      ...
      'type'                    => new sfWidgetFormChoice(array('choices' => array('image' => 'image', 'video' => 'video', 'audio' => 'audio', 'pdf' => 'pdf', 'office' => 'office'))),
    ...
    $this->setValidators(array(
      ...
      'type'                    => new sfValidatorChoice(array('choices' => array(0 => 'image', 1 => 'video', 2 => 'audio', 3 => 'pdf', 4 => 'office'))),
      ...
      'md5'                     => new sfValidatorString(array('max_length' => 32, 'required' => false)),
  ...
  public function updateDefaultsFromObject()
  {
    ...
    if (isset($this->widgetSchema['a_search_documents_list']))
    {
      $this->setDefault('a_search_documents_list', $this->object->aSearchDocuments->getPrimaryKeys());
    }
  ...
  protected function doSave($con = null)
  {
    ...
    $this->saveaSearchDocumentsList($con);
  ...
  public function saveaSearchDocumentsList($con = null)
  {
    if (!$this->isValid())
    {
      throw $this->getErrorSchema();
    ...
    $existing = $this->object->aSearchDocuments->getPrimaryKeys();
    $values = $this->getValue('a_search_documents_list');
    if (!is_array($values))
    {
      $values = array();
    }
    ...
    if (count($unlink))
    {
      $this->object->unlink('aSearchDocuments', array_values($unlink));
    }

    $link = array_diff($values, $existing);
    if (count($link))
    {
      $this->object->link('aSearchDocuments', array_values($link));
    }
  }
